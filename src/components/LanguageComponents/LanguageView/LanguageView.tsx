...
import { fetchLanguageByName } from "@/services/fetchLanguageByName";
...
const LanguageView: React.FC<{ slug: string }> = ({ slug }: { slug: string }): React.ReactElement => {
    ...
    useEffect(() => {
        const fetchLanguage = async () => {
            try {
                const fetchLanguage = await fetchLanguageByName(slug);
                ...
                loading ? <Loading /> : (
                    <div className="w-full sm:px-[16px] flex flex-col gap-16">
                        ...
                        <div className="flex flex-col items-center gap-[40px]">
                            <Subtitle label={`¿Por qué te gustaria estudiar ${language?.name}?`} />
